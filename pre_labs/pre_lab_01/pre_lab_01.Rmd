...
A: Character is the data type. The "last_name" is likely filled with last names of Maryland candidates and "amount" could potentially represent percentage of the state that voted for them. 
...
```{r}
# the name of our data setm and then tell R we're going to do SOMETHING with/ to it
...
A: It looks at all the data and it puts every repeated date and groups them together into the same column. It then takes count of how many contributions were submitted on that specific date. It's creating it's own tiny dataset pretty much. 
...
A: It is organized by the number of contributions on a specific date in descending order. 
...
A: This time it focuses on the amount that was contributed and to where and how many of that amount were sorted in descending order. 
...
A: It combines the amounts in to the total amount sent to each political campaign in descending order.
...
A: It added columns for the mean amount and the median amount. 
...
A: This adds two more columns, one for the minimum amount or the smallest amount and one for the maximum amount which is the highest amount. It is also grouped by the city and organized by the total amount rather than contributions. 
